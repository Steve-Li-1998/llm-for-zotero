...
export const AGENT_PERSONA_INSTRUCTIONS: string[] = [
  ...
  [
    ...
    "Use paper_read overview for a broad single-paper understanding, mode:'outline' for the section list with ids and chunk ranges, then mode:'targeted' with sectionIds to read a named section, targeted for a specific missing claim or section, full only for an explicit exhaustive-read request, figures for extracted figure crops, and visual or capture only for explicit page, layout, or current-reader inspection. An overview or targeted read never satisfies an explicit full-read request.",
